/* global jQuery:false */
/* global GLOBALLOGISTICS_STORAGE:false */

(function() {
	"use strict";

	function globallogistics_skin_post_header_init() {
	...
	globallogistics_add_filter( 'trx_addons_filter_skills_pie_options', function(opt) {
	...
	globallogistics_add_filter( 'trx_addons_filter_slider_init_args', function( $param, $init ) {
	...
	function skin_blogger_toggle_filter_add_element() {
		if ( jQuery('.content').find(".sc_style_toggle").length > 0 ) {
			jQuery(" .sc_style_toggle").each(function() {
				var toggle_title = GLOBALLOGISTICS_STORAGE['toggle_title'];
						...
						$share_items
							...
							.on('click', '.social_item[data-copy-link-url]', function(e) {
								...
								if ( url != '' ) {
									...
									var msg = $self.data('message') ? $self.data('message') : GLOBALLOGISTICS_STORAGE['msg_copied'];
									if ( msg ) {
	...
	// Audio
	function globallogistics_skin_audio_init() {
	...
	jQuery( window ).load(function() {
		...
		// Audio
		globallogistics_skin_audio_init();
		...
		globallogistics_skin_post_header_init();

	});

	jQuery( document ).on(
		'action.ready_globallogistics', function() {
			...
			jQuery('.tribe-events-view').on( 'beforeAjaxComplete.tribeEvents beforeAjaxSuccess.tribeEvents beforeAjaxError.tribeEvents', globallogistics_tribe_events_after_ajax );
			function globallogistics_tribe_events_after_ajax( jqXHR, textStatus ) {
				setTimeout( function() {
					...
					btn.on( 'click', function(e) {
						...
						if ( $i_agree.length > 0 ) {
							if ( ! $i_agree.get(0).checked ) {
								form.append( '<div class="trx_addons_message_box trx_addons_message_box_error">' + GLOBALLOGISTICS_STORAGE['msg_i_agree_error'] + '</div>' );
						...
						var rez = globallogistics_form_validate(form, {
							...
							rules: [
								{
									field: 'EMAIL',
									min_length: {value: 7, message: GLOBALLOGISTICS_STORAGE['mc4wp_msg_email_min']},
									max_length: {value: 60, message: GLOBALLOGISTICS_STORAGE['mc4wp_msg_email_max']},
									mask: {
			...
			globallogistics_add_filter( 'globallogistics_filter_mobile_menu_close_timeout', function( timeout, $menu ) {
				return 1000;
			} );
            ...
            function globallogistics_skin_open_panel( id ) {
            ...
            jQuery( '.trx_addons_panel_menu_link:not(.panel_inited)' )
				...
				.on( 'click', function(e) {
					globallogistics_skin_open_panel( jQuery(this).attr('href').substring(1) );
					e.preventDefault();
					return false;
				});

            // Close panel
            function globallogistics_skin_close_panel() {
            ...
            $document
                .on( 'keyup', function(e) {
					...
					if (e.keyCode === 27) {
						if  ($panel.length === 1) {
							globallogistics_skin_close_panel();
							e.preventDefault();
							return false;
						}
					}
        ...
        jQuery( $search_wrap.each( function() {
            ...
            search_field.on('keyup', function(e) {
                if (e.keyCode === 27) {
                    ...
                    setTimeout(function () {
                        globallogistics_search_close($search_wrap_init);
    ...
    function globallogistics_search_close($search_wrap_init) {
	...
	jQuery( document ).on(
		'action.scroll_globallogistics', function() {
	...
	jQuery( document ).on(
		'action.resize_globallogistics', function() {
			extra_margin_top_video_popup_button();
			globallogistics_skin_post_header_init();
			skin_add_custom_width_filter_list();
		}
	...
	jQuery( '.globallogistics_woocommerce_filters_button,.sidebar > .sidebar_close,.sidebar_overlay' ).on( 'click', function(e) {
